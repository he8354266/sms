 ...
 * @description zkjy
 * @updateUser
 * @createDate 2022/2/18 15:12
 * @updateDate 2022/2/18 15:12
 **/
@Service
@Slf4j
public class BlackListServiceImpl extends ServiceImpl<BlackListMapper, BlackListEntity> implements BlackListService {
    ...
    public R upload(MultipartFile file) {
        ...
        for (int i = 0; i < total; i++) {
            ...
            try {
                int flag = baseMapper.insert(BlackListEntity.builder().type("1").content(item.getMobile()).remark(item.getRemark()).build());
                if (flag <= 0) {
                    duplicateErrorMsg.add("第" + (i + 1) + "条");
                }
            } catch (Exception e) {
                if (e.getClass().equals("org.springframework.dao.DuplicateKeyException")) {
                    duplicateErrorMsg.add("第" + (i + 1) + "条");
                } else {
